...
async function verificarLembrete25() {
  ...
  // Busca pedidos confirmados/enviados criados entre 24 e 26 dias atrás
  ...
  try {
    const res = await fetch(
      `${SUPABASE_URL_P}/rest/v1/pedidos_online?created_at=gte.${de}&created_at=lte.${ate}&status=in.(confirmado,enviado)&select=id,nome,telefone,status`,
      { headers: { 'apikey': SUPABASE_KEY_P, 'Authorization': `Bearer ${SUPABASE_KEY_P}` } }
    );
    ...
    for (const p of pedidos) {
      ...
      try {
        ...
        console.log(`[Lembrete 25d] enviado para ${p.nome} (${tel})`);
      } catch(e) {
        console.warn('[Lembrete 25d] erro:', e);
      }
    }
  } catch(e) {
    console.warn('[Lembrete 25d] erro ao buscar pedidos:', e);
  }
}
